...
using Skinny.Monitoring.TestHelper;
using Xunit.Extensions;

namespace Skinny.Monitoring.SqlServerPublisher.Tests
{
    public class when_publish_an_event_to_the_sql_server_publisher_using_an_existing_connection : Specification
    {
        ...
        {
            given_the_skinny_monitoring_database_is_empty();

            connection = new SqlConnection(ConfigurationManager.ConnectionStrings["skinny.sqlserver"].ConnectionString);
            connection.Open();
        ...
        private void given_the_skinny_monitoring_database_is_empty()
        {
            using (
                var delete_connection =
                    new SqlConnection(ConfigurationManager.ConnectionStrings["skinny.sqlserver"].ConnectionString))
            {
        ...
        {
            using (var query_connection = new SqlConnection(ConfigurationManager.ConnectionStrings["skinny.sqlserver"].ConnectionString))
            {
