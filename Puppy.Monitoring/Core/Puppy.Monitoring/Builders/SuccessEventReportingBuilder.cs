    ...
    internal class SuccessEventReportingBuilder : IBuildReportingEvent
    {
        ...
        public IEvent Build()
        {
            return subBuilder != null
                       ? subBuilder.Build()
                       : new SuccessEvent(new Categorisation(info.Category, info.SubCategory, info.Segment), info.CorrelationId, new EventDescription(info.Parameters, info.Description));
        }
    }
}